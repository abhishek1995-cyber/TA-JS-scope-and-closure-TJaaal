...
function objOfMatches(array1, array2, cb) {
 return array2.reduce((acc,cv,index) =>{
  if(cv === cb(array1[index])){
    acc[array1[index]] = cv
  }
  return acc
 },{});
...
function multiMap(arrVals, arrCallbacks) {
  return arrVals.reduce((acc,cv)=>{
    acc[cv] = arrCallbacks.reduce((cbacc,iv)=>{
      cbacc.push(iv(cv));
      return cbacc
    },[])
    return acc
...
function objOfMatchesWithArray(array1, array2, callback) {
  return array1.reduce((acc,cv,index)=>{
    let val = callback.reduce((cacc,fn)=>fn(cacc),cv);
    if(val=== array2[index]){
      acc[cv]  = val;
    }
    return acc;
...
```js
function schedule(arrayOfFn,array) {
  if(arrayOfFn.length != array.length){
    alert("invalid input")
  }
  // arrayOfFn.map(elm =>{
  //   array.map(second =>{
  //     setTimeout(elm, second*1000)
  //   })
  // })
  for(let elm of arrayOfFn ){
    for(let elmA of array ){
      setTimeout(()=>{
        elm()
      },elmA*1000)
    }
